...
class RefCondDenoiser:
    """Wraps a denoiser so that, each denoising step, the encoded reference-audio
    latents are concatenated onto the audio state as clean context (denoise_mask 0)
    and the reference tokens are sliced back off the prediction. This is what turns
    the base text->audio model into a reference-conditioned (ScenA) generator.
    """

    def __init__(self, inner_denoiser: FactoryGuidedDenoiser, ref_conds: dict, max_ref_conds: int) -> None:
        self.inner = inner_denoiser
        self.ref_conds = ref_conds
        self.max_ref_conds = max_ref_conds

    def __call__(self, transformer, video_state, audio_state, sigmas, step_index):
        import dataclasses

        from ltx_core.pipeline.ref_conds import prepare_ref_conds_inputs, slice_ref_conds_output

        if audio_state is None or self.max_ref_conds == 0:
            return self.inner(transformer, video_state, audio_state, sigmas, step_index)

        original_seq_len = audio_state.latent.shape[1]
        ext_latent, ext_positions, _ext_timesteps, ref_cond_ids = prepare_ref_conds_inputs(
            latents=audio_state.latent,
            positions=audio_state.positions,
            timesteps=timesteps_from_mask(audio_state.denoise_mask, sigmas[step_index]),
            ref_conds=self.ref_conds,
            max_ref_conds=self.max_ref_conds,
        )
        # References are clean context: denoise_mask 0, clean_latent = the ref latents.
        n_orig = audio_state.denoise_mask.shape[1]
        ref_mask_shape = list(audio_state.denoise_mask.shape)
        ref_mask_shape[1] = ext_latent.shape[1] - n_orig
        ref_denoise_mask = torch.zeros(
            ref_mask_shape, dtype=audio_state.denoise_mask.dtype, device=audio_state.denoise_mask.device
        )
        ext_denoise_mask = torch.cat([audio_state.denoise_mask, ref_denoise_mask], dim=1)
        ext_clean = torch.cat([audio_state.clean_latent, ext_latent[:, n_orig:]], dim=1)

        extended_state = LatentState(
            latent=ext_latent,
            denoise_mask=ext_denoise_mask,
            positions=ext_positions,
            clean_latent=ext_clean,
            attention_mask=None,
        )
        # ref_cond_ids reaches the transformer via Modality (see modality_from_latent_state).
        object.__setattr__(extended_state, "_ref_cond_ids", ref_cond_ids)

        denoised_video, denoised_audio = self.inner(transformer, video_state, extended_state, sigmas, step_index)
        if denoised_audio is not None:

            def _slice(t):
                return slice_ref_conds_output(t, original_seq_len) if isinstance(t, torch.Tensor) else t

            denoised_audio = dataclasses.replace(
                denoised_audio,
                denoised=_slice(denoised_audio.denoised),
                uncond=_slice(denoised_audio.uncond),
                cond=_slice(denoised_audio.cond),
                ptb=_slice(denoised_audio.ptb),
                mod=_slice(denoised_audio.mod),
            )
