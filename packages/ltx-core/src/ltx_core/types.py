...
class Audio:
    ...
    def save(self, path: str) -> None:
        """Write the waveform to *path* (e.g. a ``.wav`` file) via torchaudio."""
        import soundfile as sf

        wf = self.waveform.detach().cpu()
        if wf.ndim == 3:  # (B, C, T) -> (C, T)
            wf = wf[0]
        # soundfile expects (frames, channels)
        sf.write(str(path), wf.transpose(0, 1).float().numpy(), self.sampling_rate, subtype="PCM_16")
