...
class LTXModel(torch.nn.Module):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        ref_cond_embedding_type: str = "none",  # "none" | "absolute" | "rope"
        max_ref_conds: int = 0,
        ref_cond_embedding_rope_max_pos: int | None = None,
    ):
        super().__init__()
        ...
        self.ref_cond_embedding_type = ref_cond_embedding_type
        self.max_ref_conds = max_ref_conds
        self._ref_cond_rope_max_pos = ref_cond_embedding_rope_max_pos
    ...
    def _init_ref_cond_embedding(self) -> None:
        """Learned absolute positional embedding added to reference-condition tokens (ScenA)."""
        if self.ref_cond_embedding_type == "absolute" and self.max_ref_conds > 0:
            inner_dim = self.audio_inner_dim if self.model_type.is_audio_enabled() else self.inner_dim
            self.ref_cond_pos_embedding = torch.nn.Parameter(torch.randn(self.max_ref_conds, inner_dim) * 0.05)

    def _apply_ref_cond_embeddings(self, args):
        """Add each reference's learned positional embedding to its tokens (ScenA absolute ref-cond)."""
        if args is None or args.ref_cond_ids is None:
            return args
        if self.ref_cond_embedding_type == "absolute" and hasattr(self, "ref_cond_pos_embedding"):
            from dataclasses import replace as _dc_replace

            ref_indices = torch.arange(1, self.max_ref_conds + 1, device=args.x.device)
            masks = (args.ref_cond_ids[:, :, None] == ref_indices[None, None, :]).to(args.x.dtype)
            embedding = torch.einsum("btm,md->btd", masks, self.ref_cond_pos_embedding.to(args.x.dtype))
            return _dc_replace(args, x=args.x + embedding)
        return args
    ...
    def forward(
        ...
        audio_args = self._apply_ref_cond_embeddings(audio_args)
