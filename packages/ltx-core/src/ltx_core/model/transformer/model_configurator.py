...
class LTXModelConfigurator(ModelConfigurator[LTXModel]):
    ...
    def from_config(cls, config: dict, ops: TransformerOpsConfig = DEFAULT_TRANSFORMER_OPS) -> LTXModel:
        ...
        return LTXModel(
            ...
            ref_cond_embedding_type=config.get("ref_cond_embedding_type", "none"),
            max_ref_conds=config.get("max_ref_conds") or 3,
            ref_cond_embedding_rope_max_pos=config.get("ref_cond_embedding_rope_positional_embedding_max_pos"),
        )
...
def _audio_only_remap_key(key: str) -> str | None:
    """Remap ScenA audio-only checkpoint keys to LTXModel AudioOnly parameter names.

    The checkpoint uses ComfyUI format (``model.diffusion_model.*`` prefix) with
    unprefixed component names; the AudioOnly model expects ``audio_``-prefixed
    parameters. ``ref_cond_pos_embedding`` is kept as-is; ``embeddings_connector.*``
    and ``text_embedding_projection*`` load into the gemma processor, not here.
    """
    ...
    _TOP_LEVEL = {
        ...
        "norm_out.": "audio_norm_out.",
        "scale_shift_table": "audio_scale_shift_table",
    }
    for prefix, replacement in _TOP_LEVEL.items():
    ...
    if key.startswith("transformer_blocks."):
        ...
        if len(parts) == 3:
            ...
            for sub, repl in _BLOCK.items():
                if rest.startswith(sub):
                    return f"{block_prefix}.{repl}{rest[len(sub):]}"

    return key


class _AudioOnlySDOps:
    """SDOps-compatible key remapper for the ScenA audio-only ref-cond checkpoint."""

    name = "LTXV_AUDIO_ONLY_RENAMING_MAP"

    def apply_to_key(self, key: str) -> str | None:
        return _audio_only_remap_key(key)

    def apply_to_key_value(self, key: str, value: torch.Tensor) -> list:
        from ltx_core.loader.sd_ops import KeyValueOperationResult  # noqa: PLC0415

        return [KeyValueOperationResult(key, value)]


LTXV_AUDIO_ONLY_RENAMING_MAP = _AudioOnlySDOps()
