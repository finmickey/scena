...
class ScenaAudioOnlyEmbeddingsProcessorConfigurator(ModelConfigurator[EmbeddingsProcessor]):
    """V1-style Gemma text readout for the ScenA audio-only checkpoint.

    A single ``aggregate_embed`` linear (no bias) over all Gemma hidden states,
    with per-segment min-max norm (config ``text_encoder_norm_type =
    PER_SEGMENT_MINMAX``). The gated connector is loaded into the
    ``video_connector`` slot (audio-only has no separate audio connector).
    """

    @classmethod
    def from_config(cls, config: dict) -> EmbeddingsProcessor:
        tc = config.get("transformer", config)
        rope_type = LTXRopeType(tc.get("rope_type", "interleaved"))
        double_precision_rope = tc.get("frequencies_precision", False) == "float64"
        connector = Embeddings1DConnector(
            num_attention_heads=tc.get("connector_num_attention_heads", 32),
            attention_head_dim=tc.get("connector_attention_head_dim", 64),
            num_layers=tc.get("connector_num_layers", 8),
            positional_embedding_max_pos=tc.get("connector_positional_embedding_max_pos", [4096]),
            num_learnable_registers=tc.get("connector_num_learnable_registers", 128),
            rope_type=rope_type,
            double_precision_rope=double_precision_rope,
            apply_gated_attention=tc.get("connector_apply_gated_attention", True),
        )
        inner_dim = connector.inner_dim
        gemma = GEMMA3_CONFIG_FOR_LTX.text_config
        flat_dim = gemma.hidden_size * (gemma.num_hidden_layers + 1)
...
SCENA_AUDIO_ONLY_EMBEDDINGS_PROCESSOR_KEY_OPS = (
    SDOps("SCENA_AUDIO_ONLY_EMBEDDINGS_PROCESSOR_KEY_OPS")
    .with_matching(prefix="text_embedding_projection.aggregate_embed.")
    .with_replacement("text_embedding_projection.aggregate_embed.", "feature_extractor.aggregate_embed.")
    .with_matching(prefix="model.diffusion_model.embeddings_connector.")
